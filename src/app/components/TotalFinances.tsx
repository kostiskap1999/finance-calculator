...
export default function TotalFinances({ financeModal, refresh, onRefresh }: { financeModal: NewFinanceProps; refresh: number; onRefresh: () => void }) {
  ...
  return (
    <div className="h-full w-full rounded-2xl p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
      <h2 className="mb-4 text-xl font-semibold text-foreground">Total Finances</h2>
      {totalFinances !== null ? (
        <p className="text-2xl font-bold text-(--accent-glow)">${totalFinances.toFixed(2)}</p>
      ) : (
        <p className="text-(--muted)">Failed to load total finances</p>
      )}
      <button
        type="button"
        onClick={() => setTempRefresh((prev) => prev + 1)}
        className="mt-4 rounded border border-(--border) px-3 py-2 text-sm text-(--muted) transition hover:bg-(--surface-elevated)"
      >
        Refresh
      </button>
    </div>
